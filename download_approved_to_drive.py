    ...
    try:
        ...
        for media_id in target_ids:
            ...
            try:
                ...
                failed += 1
                # CONFIRMED REAL need: "what's the error and where" --
                # printed at the point of failure, with the real
                # exception text, not just a count. This is the only
                # per-item message that survives the simplification
                # below; every successful item stays silent, folded
                # into the periodic progress line instead.
                print(f"{brand}: FAILED media_id={media_id!r}: {type(e).__name__}: {e}")
            finally:
            ...
            # CONFIRMED REAL simplification, direct feedback: per-item
            # noise for every single video (checking..., downloaded...,
            # uploading..., marked...) made a long run unreadable.
            # Same periodic-progress pattern already proven for email
            # scraping -- one line every 25 attempts, not one block per
            # item -- keeps a long run's live output actually scannable.
            if refunnel_export._should_print_progress(attempted, len(target_ids), 25):
                print(f"{brand}: {attempted}/{len(target_ids)} searched -- "
                      f"{uploaded} Uploaded, {status_changed} Pending, {failed} Errors")
