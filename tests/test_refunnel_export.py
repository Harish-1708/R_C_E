...
def test_format_progress_line_shows_pending_review_separately_when_present():
    ...
    # clearly labeled count, not be folded into "other error(s)".
    #
    # CONFIRMED REAL correction: this test used to accept "4 other
    # error(s)" here as correct -- but that WAS the bug. attempted
    # counts pending-review items (each loop iteration increments it
    # once, pending or not), so other_failed must subtract
    # pending_review too, or every pending post gets silently
    # double-counted: once correctly as "pending review", and again as
    # a phantom error that never actually happened. 25 - 0 - 21 - 2 = 2.
    line = _format_progress_line(attempted=25, total=25, found=0, empty_fields=21, pending_review=2)
    assert "2 other error(s)" in line  # 25 - 0 - 21 - 2, now correctly excludes pending_review
...
def test_sabotage_pending_review_counted_as_an_error_would_be_caught():
    ...
    with pytest.raises(AssertionError):
        ...
        # "other error(s)" despite zero exceptions ever happening
        assert "4 other error(s)" in line
    assert "2 other error(s)" in line
...
def test_pinned_to_the_real_run_that_exposed_this_bug():
    ...
    assert "0 other error(s)" in line
