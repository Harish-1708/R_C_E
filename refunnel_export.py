...
def goto_social_listening_for_workspace(page: Page, workspace_name: str, known_workspace_names: Iterable[str],
    ...
    just requested in the URL before the switch -- AND the Content
    sub-tab genuinely active, not whichever sub-tab (Content/Creators/
    Analytics) happened to be last selected.
    ...
    SEPARATE, CONFIRMED REAL bug also fixed here: the URL's own
    "/content/social-listening" path segment does NOT control which of
    the three Content/Creators/Analytics sub-tabs is showing -- a saved
    failure snapshot showed the page genuinely on the Creators tab
    (inline style border-bottom: 2px solid; font-weight: bold on that
    tab specifically, none on Content's) despite this exact URL, with
    zero media cards visible as a result. That sub-tab is separate,
    client-side-only state that persists from whatever was last
    selected -- by an earlier script step, or a person manually using
    the account -- and the URL alone never resets it. Every caller here
    gets the same fix: explicitly click Content after navigating,
    rather than assuming the URL guarantees it.
    """
    ...
    page.goto(refunnel_auth.refunnel_social_listening_url(usage_rights))
    _ensure_content_tab_active(page)


def _ensure_content_tab_active(page: Page) -> None:
    """Clicks the Content sub-tab ONLY if it isn't already active.

    CONFIRMED REAL regression this fixes: an earlier version of this
    function clicked unconditionally, reasoning that clicking an
    already-active tab is a harmless no-op. A live run immediately
    afterward showed a severe, unified regression -- 30/50 items
    newly flagged as "status changed since export" (Pending review)
    and 20/50 "couldn't locate on the page", both far above the prior
    rate, in the exact same run. Both symptoms match ONE mechanism:
    Refunnel's own Content-tab click handler very likely resets the
    tab to its OWN default (unfiltered) view regardless of whether it
    was already selected, silently wiping the usage_rights=GRANTED
    filter the URL had just set. Searching a ~5900-post unfiltered
    grid instead of the real ~410-post Approved one explains both the
    flood of genuine Pending-review cards AND the found-nothing
    failures (a search that only scrolls forward has much further to
    go in a 14x larger grid). Checking first and skipping the click
    when Content is already active avoids ever triggering that reset
    in the first place.
    """
    content_tab = page.locator(".new-tabs-switch:has-text('Content')").first
    try:
        is_active = "font-weight: bold" in (content_tab.get_attribute("style") or "")
    except Exception:
        is_active = False
    if not is_active:
        content_tab.click()
