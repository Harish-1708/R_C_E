...
def _format_progress_line(attempted: int, total: int, found: int, empty_fields: int = 0,
    ...
    #
    # CONFIRMED REAL bug this fixes: attempted now correctly counts
    # pending-review items (each loop iteration increments it exactly
    # once, pending or not), but this subtraction never excluded
    # pending_review from it -- so every pending-review post was
    # silently double-counted: once correctly as "pending review", and
    # AGAIN folded into "other error(s)", even with zero actual
    # exceptions anywhere in the run. A user's own complete log proved
    # this directly: 25 attempted, 0 found, 23 empty, 2 pending review
    # -- 23 + 2 already accounts for all 25, so the real error count
    # was always zero, not the 2 the old formula reported.
    ...
    return (f"scrape_creator_emails: progress {attempted}/{total} attempted "
            ...
            f"{other_failed} other error(s){pending_note}")
