...
def _ensure_content_tab_active(page: Page) -> None:
    """Clicks the Content sub-tab. See the docstring on
    goto_social_listening_for_workspace for the confirmed real
    evidence this addresses. Unconditional -- clicking an already-
    active tab is a harmless no-op on the real page, and checking the
    exact style string first would just be a more fragile way to reach
    the same result."""
    page.locator(".new-tabs-switch:has-text('Content')").first.click()
